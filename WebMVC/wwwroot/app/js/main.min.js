function initSidebar(){let e=document.body.querySelectorAll("a[data-page*='-page'");e.forEach(e=>{e.addEventListener("click",e=>{setPageByAttribute(e.target.getAttribute("data-page"))})})}function getGroupItemWithDeleteButton(e,t){if(""==t||""==e)return null;let l=document.createElement("li");l.classList.add("group__item"),l.setAttribute("data-group-id",t);let a=document.createElement("i");a.classList.add("fas","fa-minus-square"),a.addEventListener("click",e=>{e.target.closest("ul").removeChild(l)});let s=document.createElement("span");return s.innerText=e,l.insertAdjacentElement("beforeend",a),l.insertAdjacentElement("beforeend",s),l}function removeCurriculumRow(e){let t=e.closest(".curriculum__table-row-data");t.remove()}function subscribeFormElements(){let e=document.body.querySelectorAll(".form-element-field");e.forEach(e=>e.addEventListener("change",e=>{let t=e.target;t.classList[t.value?"add":"remove"]("-hasValue")}))}const optionDefault='<option class="form-select-placeholder" value="" disabled selected></option>';class Page{constructor(){}static getPage(){return"not implemented"}static init(){console.log("not initialized")}static get is(){return"Page"}static _idsToClear(){return[]}}class MainPage extends Page{constructor(){super()}static get page(){return`
        ...
        `}static init(){this._initDataObjKeyFields(),this._studentCardSubscribe(),subscribeFormElements()}static _initDataObjKeyFields(){this._dataObjKeyFields=content.querySelectorAll("[data-obj-key]")}static _studentCardSubscribe(){const e=document.querySelector(".student-card__show-btn-clear"),t=document.querySelector(".student-card__show-btn-find"),l=document.querySelector(".student-card__show-btn-edit"),a=document.querySelector(".student-card__show-btn-save"),s=document.querySelector(".student-card__show-average-score-btn"),i=document.querySelector("#group"),o=document.querySelector("#course"),n=document.querySelector("#student"),r=document.querySelector("#select-year"),c=document.querySelector("#year");e.addEventListener("click",()=>{let e=this._idsToClear(),t=document.querySelectorAll(e.join(", "));t.forEach(e=>{e.classList.remove("-hasValue"),e.value=""})}),t.addEventListener("click",()=>{!async function(e){if(""!=e){const t=await fetch(`api/StudentCard/ShowStudentInfo?studentId=${e}`),l=await t.json();StudentCardShowPage._dataObjKeyFields.forEach(e=>{e.value=l[e.getAttribute("data-obj-key")],e.classList.add("-hasValue")}),console.log(l)}}(n.value)}),l.addEventListener("click",()=>{console.log("Edit")}),a.addEventListener("click",()=>{!async function(e){let t={};StudentCardShowPage._dataObjKeyFields.forEach(e=>{t[e.getAttribute("data-obj-key")]=e.value}),console.log(t);await fetch(`api/StudentCard/UpdateStudentCardInfo?studentId=${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)})}(n.value)}),s.addEventListener("click",()=>{console.log("Average score")}),r.addEventListener("change",()=>{c.disabled=!r.checked,r.checked||(c.value="",c.classList.remove("-hasValue"),o.dispatchEvent(new Event("change")))}),c.addEventListener("change",()=>{d(o.value,c.value)}),o.addEventListener("change",()=>{r.checked&&!c.value?console.log("Error: year wasn't selected"):r.checked&&c.value?d(o.value,c.value):d(o.value)}),i.addEventListener("change",()=>{!async function(t){if(""!=t){const l=await fetch(`api/StudentCard/GetListOfStudents?groupId=${t}`),a=await l.json();let e=a.map(e=>`<option value=${e.studentId}>${e.studentName}</option>`);e.push(optionDefault),n.innerHTML=e.join(""),n.classList.remove("-hasValue")}}(i.value)});async function d(l,a=null){if(""!=a&&""!=l){let e=`api/Group/GetListOfGroups?course=${l}`;a&&(e+=`&year=${a}`);const s=await fetch(e),o=await s.json();console.log(o);let t=o.map(e=>`<option value=${e.groupId}>${e.groupName}</option>`);t.push(optionDefault),i.innerHTML=t.join(""),i.classList.remove("-hasValue"),n.value="",n.innerHTML=optionDefault,n.classList.remove("-hasValue")}}}static _idsToClear(){return["#speciality","#specialization","#year","#course","#group","#student"]}}class CurriculumAddPage extends Page{constructor(){super()}static get is(){return"curriculum--add-page"}static _counter=1;static get page(){return`
        ...
        `}static init(){this._groupCardSubscribe(),subscribeFormElements()}static _groupCardSubscribe(){const t=document.querySelector(".curriculum__find-groups"),e=document.querySelector(".curriculum__table"),l=document.querySelector(".curriculum__table-row-last"),a=document.querySelector("#add-row-btn"),o=document.querySelector("#year"),i=document.querySelector("#plan-name"),s=document.querySelector("#course"),n=document.querySelector("#group"),r=document.querySelector(".curriculum-group__btn-add"),c=document.querySelector(".curriculum__info-add-btn");o.addEventListener("change",()=>{d(s.value,o.value)}),s.addEventListener("change",()=>{d(s.value)}),r.addEventListener("click",()=>{var e=getGroupItemWithDeleteButton(n.options[group.selectedIndex].text,n.value);e&&t.insertAdjacentElement("beforeend",e)}),c.addEventListener("click",()=>{if(""==o||""==i)return alert("bad");let a={},s=e.querySelectorAll(".curriculum__table-row-data");a.subjectInfo=[];for(let e=0;e<s.length;e++){let t=s[e].querySelectorAll("[data-table-key]"),l={};for(let e=0;e<t.length;e++)l[t[e].getAttribute("data-table-key")]=t[e]["checkbox"==t[e].type?"checked":"value"];a.subjectInfo.push(l)}a.name=i.value,a.year=o.value,a.groupIds=[...t.children].map(e=>+e.getAttribute("data-group-id")).filter((e,t,l)=>l.indexOf(e)===t),console.log(a)});async function d(t,l){if(""!=t){t=`api/Group/GetListOfGroups?course=${t}`;t+=l?`&year=${l}`:"";const a=await fetch(t),s=await a.json();let e=s.map(e=>`<option value=${e.groupId}>${e.groupName}</option>`);e.push(optionDefault),n.innerHTML=e.join(""),n.classList.remove("-hasValue")}}a.addEventListener("click",()=>{var e;l.insertAdjacentHTML("beforebegin",`
                <div class="curriculum__table-row curriculum__table-row-data">
                    ...
                    <div class="item">
                        <input class="item__data-subject" data-table-key="name" type="text" value="${e?.name??""}">
                    </div>
                    ...
                    <div class="item item__col-2">
                        <input type="checkbox" data-table-key="isIndividualPlanExistFall" ${e?.fall?.isActive?"checked":""}>
                        <select name="control-form-fall" data-table-key="controlTypeFall" id="control-fall">
                            <option value="0" ${0==e?.fall?.control?"selected":""}></option>
                            <option value="1" ${1==e?.fall?.control?"selected":""}>Кредит</option>
                            <option value="2" ${2==e?.fall?.control?"selected":""}>Екзамен</option>
                        </select>
                    </div>
                    <div class="item item__col-2">
                        <input type="checkbox" data-table-key="isIndividualPlanExistSpring" ${e?.spring?.isActive?"checked":""}>
                        <select name="control-form-spring" data-table-key="controlTypeSpring" id="control-spring">
                            <option value="0" ${0==e?.spring?.control?"selected":""}></option>
                            <option value="1" ${1==e?.spring?.control?"selected":""}>Кредит</option>
                            <option value="2" ${2==e?.spring?.control?"selected":""}>Екзамен</option>
                        </select>
                    </div>
                    <div class="item">
                        <input class="item__data-department" data-table-key="department" type="text" value="${e?.department??""}">
                    </div>
                </div>
            `)})}}class GroupAddPage extends Page{constructor(){super()}static get is(){return"group--add-page"}static get page(){return`
        ...
        `}static init(){this._groupCardSubscribe(),subscribeFormElements()}static _groupCardSubscribe(){const t=document.querySelector("#select-student"),e=document.querySelector("#course"),s=document.querySelector("#group"),o=document.querySelector("#student"),i=document.querySelector(".btn-clear"),l=[document.querySelector(".transfer__btn-transfer"),document.querySelector(".transfer__btn-vacation"),document.querySelector(".transfer__btn-leave"),document.querySelector(".transfer__btn-remove"),document.querySelector(".transfer__btn-resume")];l.forEach(e=>{e.addEventListener("click",()=>{t.checked&&!o.value?console.log("Error: the student wasn't select"):o.value?async function(e){s.value,o.value;console.log(e)}(e.getAttribute("data-transfer-key")):async function(){console.log("transfer group")}(s.value)})}),i.addEventListener("click",()=>{o.classList.remove("-hasValue"),o.value="",i.classList.add("btn-hide"),a(!o.value)}),s.addEventListener("change",()=>{!async function(t){if(""!=t){const l=await fetch(`api/StudentCard/GetListOfStudents?groupId=${t}`),a=await l.json();let e=a.map(e=>`<option value=${e.studentId}>${e.studentName}</option>`);e.push(optionDefault),o.innerHTML=e.join(""),o.classList.remove("-hasValue"),i.dispatchEvent(new Event("click"))}else o.innerHTML=""}(s.value)}),t.addEventListener("click",()=>{o.disabled=!t.checked,t.checked||i.dispatchEvent(new Event("click"))}),e.addEventListener("change",()=>{!async function(t){if(""!=t){const l=await fetch(`api/Group/GetListOfGroups?course=${t}`),a=await l.json();let e=a.map(e=>`<option value=${e.groupId}>${e.groupName}</option>`);e.push(optionDefault),s.innerHTML=e.join(""),s.classList.remove("-hasValue"),i.dispatchEvent(new Event("click")),o.innerHTML=""}}(e.value)}),o.addEventListener("change",()=>{i.classList[o.value?"remove":"add"]("btn-hide"),a(!o.value)});async function a(t){l.forEach(e=>{e.classList.contains("transfer__btn-transfer")||(e.disabled=t)})}}}class CreateReportPage extends Page{constructor(){super()}static get is(){return"create-report--page"}static get page(){return`
