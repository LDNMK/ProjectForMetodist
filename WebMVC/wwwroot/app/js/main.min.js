let content=document.body.querySelector(".main__content");content||console.log("Container with class'main__content' not found!");const mapper={mainPage:()=>"not implemented",studentCardAddPage:()=>"not implemented",studentCardShowPage:()=>getStudentCardShow()};function setPageByAttribute(e){e in mapper?content.innerHTML=mapper[e]():console.log(`Tag with attribute data-page='${e}' not found!`)}let pages=document.body.querySelectorAll("a[data-page*='Page'");pages.forEach(e=>{e.addEventListener("click",e=>{setPageByAttribute(e.target.getAttribute("data-page"))})});let formItems=document.body.querySelectorAll(".form-element-field");function getStudentCardShow(){return`
    ...
    `}formItems.forEach(e=>e.addEventListener("change",e=>{let l=e.target;l.classList[l.value?"add":"remove"]("-hasValue")}));