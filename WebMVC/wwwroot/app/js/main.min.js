function initSidebar(){let e=document.body.querySelectorAll("a[data-page*='-page'"),t=document.body.querySelectorAll(".sidebar__menu-item");e.forEach(e=>{e.addEventListener("click",e=>{setPageByAttribute(e.target.getAttribute("data-page")),t.forEach(e=>e.classList.remove("active")),e.target.closest(".sidebar__menu-item").classList.add("active")})})}function getGroupItemWithButton(e,t,l){return""==t||""==e||null==l?null:`
        ...
        <div class="curriculum__table-row curriculum__table-row-data" >
            ...
            <div class="item item__col-2">
                ...
                <select name="control-form-fall" data-table-key="controlFallType" id="control-fall" ${e?"disabled":""} data-is-disabled>
                    <option value="0" ${0==e?.controlFallType?"selected":""}></option>
            ...
            <div class="item item__col-2">
                ...
                <select name="control-form-spring" data-table-key="controlSpringType" id="control-spring" ${e?"disabled":""} data-is-disabled>
                    <option value="0" ${0==e?.controlSpringType?"selected":""}></option>
    ...
    `}function getProgressStudentRow(s,e){return`
        <div class="progress__row progress__row-student" data-student-id="${s.id}">
            <div class="progress__row-cell progress__row-cell-student">
                <span>${s.name}</span>
            </div>
            <ul class="progress__row-cell progress__row-cell-marks">
                ${e.map(t=>{let e=s.subjects.find(e=>e.id==t.id);var l=e?.mark??"",a=e?.modifiedOn?e.modifiedOn.split("T")[0]:"";return console.log(a),`
            ...
            <div class="progress__row-cell progress__row-cell-final-mark">
                <span data-student-final>
                    ${Math.round(s.subjects.map(e=>e.mark)?.reduce((e,t)=>e+=t,0)/s.subjects.length)}
                </span>
            </div>
        </div>
    ...
    `}function getTransferStudentRow(e){return`
        <li class="transfer__student-row">
            ...
            <select name="status" data-transfer-key="stateId">
                <option value="1" ${1==e.stateId?"selected":""}>Перевести з групою</option>
                <option value="2" ${2==e.stateId?"selected":""}>Академ. відпустка</option>
                <option value="3" ${3==e.stateId?"selected":""}>Перехід в іншу групу</option>
                <option value="4" ${4==e.stateId?"selected":""}>Не закрив сесію</option>
                <option value="5" ${5==e.stateId?"selected":""}>Відрахований</option>
                <option value="6" ${6==e.stateId?"selected":""}>Здобув степінь</option>
            </select>
    ...
    `}function subscribeFormElements(){let e=document.body.querySelectorAll(".form-element-field");e.forEach(e=>e.addEventListener("change",e=>{let t=e.target;t.classList[t.value?"add":"remove"]("-hasValue")}))}const optionDefault='<option class="form-select-placeholder" value="" disabled selected></option>';class Page{constructor(){}static getPage(){return"not implemented"}static init(){console.log("not initialized")}static get is(){return"Page"}static _idsToClear(){return[]}}class MainPage extends Page{constructor(){super()}static get page(){return`
        ...
        `}static init(){this._initDataObjKeyFields(),this._studentCardSubscribe(),subscribeFormElements()}static _initDataObjKeyFields(){this._dataObjKeyFields=content.querySelectorAll("[data-obj-key]")}static _studentCardSubscribe(){const e=document.querySelector(".student-card__show-btn-find"),t=document.querySelector(".student-card__show-btn-edit"),l=document.querySelector(".student-card__show-btn-save"),a=document.querySelector(".student-card__show-average-score-btn"),i=document.querySelector("#group"),s=document.querySelector("#course"),r=document.querySelector("#student"),o=document.querySelector("#speciality"),n=document.querySelector("#select-year"),d=document.querySelector("#year");e.addEventListener("click",()=>{!async function(e){if(""!=e){const t=await fetch(`api/StudentCard/ShowStudentInfo?studentId=${e}`),l=await t.json();console.log(l),StudentCardShowPage._dataObjKeyFields.forEach(e=>{e.value=l[e.getAttribute("data-obj-key")],e.value&&e.classList.add("-hasValue")}),console.log(l)}}(r.value)}),t.addEventListener("click",()=>{this._dataObjKeyFields.forEach(e=>{e.disabled=!e.disabled})}),l.addEventListener("click",()=>{!async function(e){let t={};StudentCardShowPage._dataObjKeyFields.forEach(e=>{t[e.getAttribute("data-obj-key")]=e.value}),console.log(t);await fetch(`api/StudentCard/UpdateStudentCardInfo?studentId=${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)})}(r.value)}),a.addEventListener("click",()=>{console.log("Average score")}),n.addEventListener("change",()=>{d.disabled=!n.checked,n.checked||(d.value="",d.classList.remove("-hasValue"),s.dispatchEvent(new Event("change")))}),d.addEventListener("change",()=>{c(s.value,d.value)}),s.addEventListener("change",()=>{n.checked&&!d.value?console.log("Error: year wasn't selected"):n.checked&&d.value?c(s.value,d.value):c(s.value)}),i.addEventListener("change",()=>{!async function(t){if(""!=t){const l=await fetch(`api/StudentCard/GetListOfStudents?groupId=${t}`),a=await l.json();let e=a.map(e=>`<option value=${e.studentId}>${e.studentName}</option>`);e.push(optionDefault),r.innerHTML=e.join(""),r.classList.remove("-hasValue")}}(i.value)});async function c(l,a=null){if(""!=a&&""!=l){let e=`api/Group/GetGroups?course=${l}`;a&&(e+=`&year=${a}`);const s=await fetch(e),o=await s.json();console.log(o);let t=o.map(e=>`<option value=${e.groupId}>${e.groupName}</option>`);t.push(optionDefault),i.innerHTML=t.join(""),i.classList.remove("-hasValue"),r.value="",r.innerHTML=optionDefault,r.classList.remove("-hasValue")}}window.onload=async function(){const e=await fetch("api/StudentCard/GetSpecialities"),t=await e.json();let l=t.map(e=>`<option value=${e.id}>${e.name}</option>`);l.push(optionDefault),o.innerHTML=l.join(""),o.classList.remove("-hasValue")}()}static _idsToClear(){return["#speciality","#specialization","#year","#course","#group","#student"]}}class CurriculumAddPage extends Page{constructor(){super()}static get is(){return"curriculum--add-page"}static _counter=1;static get page(){return`
        ...
        `}static init(){this._groupCardSubscribe(),subscribeFormElements()}static buttons={remove:"<i class='remove fas fa-minus-square' onClick='CurriculumAddPage.removeGroupFromList(this)'></i>"};static removeGroupFromList(e){const t=e.closest(".group__item");t.remove()}static _groupCardSubscribe(){const t=document.querySelector(".curriculum__find-groups"),e=document.querySelector(".curriculum__table"),l=document.querySelector(".curriculum__table-row-last"),a=document.querySelector("#add-row-btn"),o=document.querySelector("#year"),i=document.querySelector("#plan-name"),s=document.querySelector("#course"),r=document.querySelector("#group"),n=document.querySelector(".curriculum-group__btn-add"),d=document.querySelector(".curriculum__info-add-btn");o.addEventListener("change",()=>{console.log("year")}),s.addEventListener("change",()=>{!async function(t){if(""!=t){const l=await fetch(`api/Group/GetGroups?course=${t}`),a=await l.json();let e=a.map(e=>`<option value=${e.groupId}>${e.groupName}</option>`);e.push(optionDefault),r.innerHTML=e.join(""),r.classList.remove("-hasValue")}}(s.value)}),n.addEventListener("click",()=>{var e=getGroupItemWithButton(r.options[group.selectedIndex].text,r.value,CurriculumAddPage.buttons.remove);e&&t.insertAdjacentHTML("beforeend",e)}),d.addEventListener("click",()=>{if(""==o||""==i)return alert("bad");let a={},s=e.querySelectorAll(".curriculum__table-row-data");a.subjectInfo=[];for(let e=0;e<s.length;e++){let t=s[e].querySelectorAll("[data-table-key]"),l={};for(let e=0;e<t.length;e++)l[t[e].getAttribute("data-table-key")]=t[e]["checkbox"==t[e].type?"checked":"value"];a.subjectInfo.push(l)}a.name=i.value,a.year=o.value,a.groupIds=[...t.children].map(e=>+e.getAttribute("data-group-id")).filter((e,t,l)=>l.indexOf(e)===t),console.log(a),async function(e){e=await fetch("api/YearPlan/AddYearPlan",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)});console.log(e)}(a)});a.addEventListener("click",()=>{l.insertAdjacentHTML("beforebegin",addCurriculumRow())})}}class CurriculumShowPage extends Page{constructor(){super()}static get is(){return"curriculum--show-page"}static get page(){return`
            <div class="curriculum__grid">
        ...
        `}static init(){this._subscribePageElements(),subscribeFormElements()}static _subscribePageElements(){const t=document.querySelector(".curriculum__table"),s=document.querySelector(".curriculum__table-row-last"),e=document.querySelector("#add-row-btn"),o=document.querySelector("#plan-id"),i=document.querySelector("#plan-name"),l=document.querySelector("#year"),a=document.querySelector("#course"),r=document.querySelector("#group"),n=document.querySelector(".curriculum__btn-find"),d=document.querySelector(".curriculum-info__btn-edit"),c=document.querySelector(".curriculum-info__btn-save");l.addEventListener("change",()=>{u(a.value,l.value)}),a.addEventListener("change",()=>{u(a.value,l.value)}),n.addEventListener("click",()=>{!async function(e,t){if(""!=e){const l=await fetch(`api/YearPlan/GetYearPlanByGroup?groupId=${e}&year=${t}`);if(console.log(l),l.ok){const a=await l.json();console.log(a),o.value=a.id,o.classList.add("-hasValue"),i.value=a.name,i.classList.add("-hasValue"),a.subjectInfo.forEach(e=>{s.insertAdjacentHTML("beforebegin",addCurriculumRow(e))})}else console.log("Year plan not found")}}(r.value,l.value)}),d.addEventListener("click",()=>{let e=t.querySelectorAll("[data-is-disabled]");e.forEach(e=>{e.disabled=!e.disabled})}),c.addEventListener("click",()=>{!async function(e){if(""!=e){let a={id:e,name:i.value,year:l.value,groupdIds:[r.value],subjectInfo:[]},s=t.querySelectorAll(".curriculum__table-row-data");for(let e=0;e<s.length;e++){let t=s[e].querySelectorAll("[data-table-key]"),l={};for(let e=0;e<t.length;e++)l[t[e].getAttribute("data-table-key")]=t[e]["checkbox"==t[e].type?"checked":"value"];a.subjectInfo.push(l)}console.log(a);await fetch(`api/YearPlan/UpdateYearPlan?yearPlanId=${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)})}}(o.value)});async function u(t,l){if(""!=t){t=`api/Group/GetGroups?course=${t}`;t+=l?`&year=${l}`:"";const a=await fetch(t),s=await a.json();let e=s.map(e=>`<option value=${e.groupId}>${e.groupName}</option>`);e.push(optionDefault),r.innerHTML=e.join(""),r.classList.remove("-hasValue")}}e.addEventListener("click",()=>{e.hasAttribute("disabled")||s.insertAdjacentHTML("beforebegin",addCurriculumRow())})}}class GroupAddPage extends Page{constructor(){super()}static get is(){return"group--add-page"}static get page(){return`
        ...
        `}static init(){this._groupCardSubscribe(),subscribeFormElements()}static _groupCardSubscribe(){const s=document.querySelector(".transfer__students-list"),e=document.querySelector("#year"),t=document.querySelector("#course"),o=document.querySelector("#group"),l=document.querySelector("#btn-find"),a=document.querySelector("#btn-transfer");t.addEventListener("change",()=>{!async function(t){if(""!=t){const l=await fetch(`api/Group/GetGroups?course=${t}`),a=await l.json();let e=a.map(e=>`<option value=${e.groupId}>${e.groupName}</option>`);e.push(optionDefault),o.innerHTML=e.join(""),o.classList.remove("-hasValue")}else console.log("Course is empty")}(t.value)}),l.addEventListener("click",()=>{!async function(e,t){if(""!=e&&""!=t){const l=await fetch(`api/Transfer/GetStudentsForTransfer?groupId=${e}&year=${t}`),a=await l.json();s.innerHTML="",s.insertAdjacentHTML("beforeend",getTransferStudentHead()),a.forEach(e=>{s.insertAdjacentHTML("beforeend",getTransferStudentRow(e))})}else s.innerHTML=""}(o.value,e.value)}),a.addEventListener("click",()=>{!async function(){let a=[];const e=[...s.querySelectorAll(".transfer__student-row")];e.forEach(e=>{const t=e.querySelectorAll("[data-transfer-key]");let l={};t.forEach(e=>{l[e.getAttribute("data-transfer-key")]="SPAN"==e.tagName?e.innerText:e.value}),a.push(l)}),console.log(a)}()})}}class CreateReportPage extends Page{constructor(){super()}static get is(){return"create-report--page"}static get page(){return`
        ...
        `}static init(){this._groupCardSubscribe(),subscribeFormElements()}static _groupCardSubscribe(){const e=document.querySelector("#course"),t=document.querySelector("#year"),i=document.querySelector("#group"),r=document.querySelector("#student"),l=document.querySelector(".create-report__create-btn");e.addEventListener("change",()=>{console.log("course"),t.value?a(e.value,t.value):a(e.value)}),t.addEventListener("change",()=>{t.value?a(e.value,t.value):a(e.value)}),i.addEventListener("change",()=>{!async function(t){if(""!=t){const l=await fetch(`api/StudentCard/GetListOfStudents?groupId=${t}`),a=await l.json();let e=a.map(e=>`<option value=${e.studentId}>${e.studentName}</option>`);e.push(optionDefault),r.innerHTML=e.join(""),r.classList.remove("-hasValue")}}(i.value)}),l.addEventListener("click",()=>{!async function(e){if(""!=e){const t=await fetch(`api/Report/CreateReport?studentId=${e}`);await t.json()}}(r.value)});async function a(l,a=null){if(""!=l){let e=`api/Group/GetGroups?course=${l}`;a&&(e+=`&year=${a}`);const s=await fetch(e),o=await s.json();console.log(o);let t=o.map(e=>`<option value=${e.groupId}>${e.groupName}</option>`);t.push(optionDefault),i.innerHTML=t.join(""),i.classList.remove("-hasValue"),r.value="",r.innerHTML=optionDefault,r.classList.remove("-hasValue")}}}}class ProgressPage extends Page{constructor(){super()}static get is(){return"progress--page"}static get page(){return`
        ...
        `}static init(){this._subscribePageElements(),subscribeFormElements()}static _subscribePageElements(){const e=document.querySelector("#year"),t=document.querySelector("#course"),o=document.querySelector("#group"),l=document.querySelector("#semestr"),i=document.querySelector(".progress__table"),r=document.querySelector(".progress__row-cell-subjects"),a=document.querySelector(".progress__btn-find"),s=document.querySelector(".progress__btn-edit"),n=document.querySelector(".progress__btn-save");e.addEventListener("change",()=>{}),t.addEventListener("change",()=>{!async function(t,l){if(""!=t){t=`api/Group/GetGroups?course=${t}`;t+=l?`&year=${l}`:"";const a=await fetch(t),s=await a.json();let e=s.map(e=>`<option value=${e.groupId}>${e.groupName}</option>`);e.push(optionDefault),o.innerHTML=e.join(""),o.classList.remove("-hasValue")}}(t.value)}),a.addEventListener("click",()=>{!async function(e,t,l){if(""!=e&&""!=t&&""!=l){!function(){let l=i.querySelectorAll(".progress__row-cell-subject");for(let e=0,t=l.length;e<t;e++)l[e].remove();let a=i.querySelectorAll(".progress__row-student");for(let e=0,t=a.length;e<t;e++)a[e].remove()}();l=`api/Progress/GetProgress?year=${e}&groupId=${t}&semesterId=${l}`;console.log(l);const a=await fetch(l),s=await a.json();s.subjects.forEach(e=>{r.insertAdjacentHTML("beforeend",getProgressSubjectCell(e))}),s.students.forEach(e=>{i.insertAdjacentHTML("beforeend",getProgressStudentRow(e,s.subjects))}),console.log(s)}}(e.value,o.value,l.value)}),s.addEventListener("click",()=>{let e=i.querySelectorAll(".progress__row-cell-marks input");e.forEach(e=>{e.disabled=!e.disabled})}),n.addEventListener("click",()=>{let l=[],e=i.querySelectorAll(".progress__row-student");e.forEach(e=>{let t={id:e.getAttribute("data-student-id"),name:e.querySelector(".progress__row-cell-student span")?.innerText};t.subjects=[...e.querySelectorAll(".progress__row-cell-mark")].map(e=>{let t=e.querySelector(".progress__row-cell-mark-value");e=e.querySelector(".progress__row-cell-mark-date");return{id:+t.getAttribute("data-subject-id"),mark:""==t.value?null:t.value,modifiedOn:""==e.value?null:e.value}}),l.push(t)}),async function(e){console.log(e);e=await fetch("api/Progress/UpdateProgress",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)});console.log(e)}(l)})}}let content=document.body.querySelector(".main__content");content||console.log("Tag with class'main__content' not found!");const mapper={"main--page":MainPage,"student-card--add-page":StudentCardAddPage,"student-card--show-page":StudentCardShowPage,"curriculum--show-page":CurriculumShowPage,"curriculum--add-page":CurriculumAddPage,"transfer--page":TransferPage,"progress--page":ProgressPage,"create-report--page":CreateReportPage,"group--add-page":GroupAddPage,"group--actualize-page":GroupActualizePage};function setPageByAttribute(t){if(t in mapper){let e=mapper[t];content.innerHTML=e.page,e.init()}else console.log(`Tag with attribute data-page='${t}' not found!`)}function init(){initSidebar(),setPageByAttribute(ProgressPage.is)}init();