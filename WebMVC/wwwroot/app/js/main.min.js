function initSidebar(){let e=document.body.querySelectorAll("a[data-page*='-page'");e.forEach(e=>{e.addEventListener("click",e=>{setPageByAttribute(e.target.getAttribute("data-page"))})})}function subscribeFormElements(){let e=document.body.querySelectorAll(".form-element-field");e.forEach(e=>e.addEventListener("change",e=>{let l=e.target;l.classList[l.value?"add":"remove"]("-hasValue")}))}const optionDefault='<option class="form-select-placeholder" value="" disabled selected></option>';class Page{constructor(){}static getPage(){return"not implemented"}static init(){console.log("not initialized")}static get is(){return"Page"}static _idsToClear(){return[]}}class MainPage extends Page{constructor(){super()}static get page(){return`
        ...
        `}static init(){this._initDataObjKeyFields(),this._studentCardSubscribe(),subscribeFormElements()}static _initDataObjKeyFields(){this._dataObjKeyFields=content.querySelectorAll("[data-obj-key]")}static _studentCardSubscribe(){const e=document.querySelector(".create-group__btn-save"),l=document.querySelector("#year"),t=document.querySelector("#name");e.addEventListener("click",()=>{!async function(e,l){""!=e&&""!=l&&(e={groupName:l},console.log(e),await fetch(`api/Group/CreateGroup?groupName=${l}`,{method:"POST"}))}(l.value,t.value)})}}let content=document.body.querySelector(".main__content");content||console.log("Tag with class'main__content' not found!");const mapper={"main--page":MainPage,"student-card--add-page":StudentCardAddPage,"student-card--show-page":StudentCardShowPage,"group--add-page":GroupAddPage};function setPageByAttribute(l){if(l in mapper){let e=mapper[l];content.innerHTML=e.page,e.init()}else console.log(`Tag with attribute data-page='${l}' not found!`)}function init(){initSidebar(),setPageByAttribute(GroupAddPage.is)}init();