const content=document.body.querySelector(".content");class ApiHelper{async _callApi(e,t){const l=await fetch(e,t);console.log("Response API"),console.log(l);t=await l.json();if(console.log("Response JSON"),console.log(t),!t.notificationType)return t;NotificationHelper.addNotification(t.notificationType,t.notificationText)}async fetchCreateGroup(e){await this._callApi(`api/Group/CreateGroup?groupName=${e}`,{method:"POST"})}async fetchGetGroups(e,t){return await this._callApi(`api/Group/GetGroups?course=${e}${t?`&year=${t}`:""}`)}async fetchGetStudents(e,t){return await this._callApi(`api/StudentCard/GetStudents?groupId=${e}${t?`&year=${t}`:""}`)}async fetchGetStudent(e){return await this._callApi(`api/StudentCard/ShowStudentInfo?studentId=${e}`)}async fetchCreateReport(e){await this._callApi(`api/Report/CreateReport?studentId=${e}`)}async fetchCreateYearPlan(e){await this._callApi("api/YearPlan/AddYearPlan",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)})}async fetchUpdateYearPlan(e,t){await this._callApi(`api/YearPlan/UpdateYearPlan?yearPlanId=${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)})}async fetchGetYearPlan(e,t){return await this._callApi(`api/YearPlan/GetYearPlanByGroup?groupId=${e}&year=${t}`)}async fetchGetProgress(e,t,l){return await this._callApi(`api/Progress/GetProgress?year=${e}&groupId=${t}&semesterId=${l}`)}async fetchUpdateProgress(e){await this._callApi("api/Progress/UpdateProgress",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)})}async fetchGetSpecialities(e){return await this._callApi(`api/StudentCard/GetSpecialities?degreeId=${e}`)}async fetchStudentCardSave(e){await this._callApi("api/StudentCard/SaveStudentCardInfo",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)})}async fetchStudentCardUpdate(e,t){await this._callApi(`api/StudentCard/UpdateStudentCardInfo?studentId=${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)})}async fetchGetStudentsForTransfer(e,t){return await this._callApi(`api/Transfer/GetStudentsForTransfer?groupId=${e}&year=${t}`)}async fetchUpdateTransferStudents(e){await this._callApi("api/Transfer/TransferStudents",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)})}}const notificationContainer=document.querySelector(".notification__list"),notificationSettings={SuccessEnum:{timeout:5e3,title:"Успішно",icon:'<i class="success-icon fas fa-check-circle"></i>',type:"success"},WarningEnum:{timeout:6e3,title:"Попередження",icon:'<i class="warning-icon fas fa-exclamation-circle"></i>',type:"warning"},ErrorEnum:{timeout:6e3,title:"Помилка",icon:'<i class="error-icon fas fa-times-circle"></i>',type:"error"}};class NotificationHelper{static addNotification(e,t){let l=notificationSettings[e];l.text=t;let a=this._getNotificationElement(l);notificationContainer.insertAdjacentElement("beforeend",a),setTimeout(()=>{a.classList.remove("showNotification"),setTimeout(()=>{a.remove()},700)},l.timeout)}static _getNotificationElement(e){let t=`
        ...
        `;return this._createElementFromHTML(t.trim())}static _createElementFromHTML(e){var t=document.createElement("div");return t.innerHTML=e.trim(),t.firstChild}}const mask=document.body.querySelector(".loading-mask");function toggleMask(){mask.classList.toggle("show")}function validateFields(e,t){let l=0;return e.forEach(e=>{e.value||(e.closest(".form-element").classList.add("form-has-error"),++l)}),t?0<t:!(0<content.querySelectorAll(".form-has-error").length)||(NotificationHelper.addNotification("ErrorEnum","Необхідно заповнити усі поля"),!1)}const apiHelper=new ApiHelper;async function getGroupsAsOptions(e,t){let l=[];const a=await apiHelper.fetchGetGroups(e,t);return a&&(l=a.map(e=>`<option value=${e.groupId}>${e.groupName}</option>`),l.push(optionDefault)),l}async function getStudentsAsOptions(e,t){let l=[];const a=await apiHelper.fetchGetStudents(e,t);return a&&(l=a.map(e=>`<option value=${e.studentId}>${e.studentName}</option>`),l.push(optionDefault)),l}async function getSpecialitiesAsOptions(e){let t=[];const l=await apiHelper.fetchGetSpecialities(e);return l&&(t=l.map(e=>`<option value=${e.id}>${e.name}</option>`),t.push(optionDefault)),t}function initSidebar(){let e=document.body.querySelectorAll("a[data-page*='-page'"),t=document.body.querySelectorAll(".sidebar__menu-item");e.forEach(e=>{e.addEventListener("click",e=>{setPageByAttribute(e.target.getAttribute("data-page")),t.forEach(e=>e.classList.remove("active")),e.target.closest(".sidebar__menu-item").classList.add("active")})})}function getGroupItemWithButton(e,t,l){return""==t||""==e||null==l?null:`
    ...
    `}function getProgressStudentRow(i,e){return`
        <div class="progress__row progress__row-student" data-student-id="${i.id}">
            ...
            <ul class="progress__row-cell progress__row-cell-marks">
                ${e.map(t=>{let e=i.subjects.find(e=>e.id==t.id);var l=e?.mark??"",a=e?.modifiedOn?e.modifiedOn.split("T")[0]:"",s=e?.taskMark??"";return console.log(a),`
                            <li class="progress__row-cell progress__row-cell-mark${t.isCourseWorkExist?"-with-cw":""}" >
                                <div style="display: flex; flex-direction: column;" subject-mark>
                                    <input class="progress__row-cell-mark-value" type="text" value="${l}" pattern="[0-9]" data-subject-id="${t.id}" disabled>
                                    <input class="progress__row-cell-mark-date" type="date" value="${a}" disabled/>
                                </div>
                                <div subject-cw>
    ...
    `}function getTransferStudentRow(e){return`
        <li class="transfer__student-row ${e.isActive?"active":""}">
            ...
            <select name="status" data-transfer-key="stateId" ${e.isActive?"":"disabled"}>
    ...
    `}function isFieldEmpty(e){let t=e.target;return t.closest(".form-element").classList[t.value?"remove":"add"]("form-has-error"),!t.value}function subscribeFormElements(){let e=document.body.querySelectorAll(".form-element-field");e.forEach(e=>e.addEventListener("change",e=>{let t=e.target;t.classList[t.value?"add":"remove"]("-hasValue")}))}const optionDefault='<option class="form-select-placeholder" value="" disabled selected></option>';class Page{constructor(){}static getPage(){return"not implemented"}static init(){console.log("not initialized")}static get is(){return"Page"}static _idsToClear(){return[]}}class MainPage extends Page{constructor(){super()}static get page(){return`
        ...
        `}static init(){this._initDataObjKeyFields(),this._studentCardSubscribe(),subscribeFormElements()}static _initDataObjKeyFields(){this._dataObjKeyFields=content.querySelectorAll("[data-obj-key]"),this._requiredFields=[...this._dataObjKeyFields].filter(e=>e.hasAttribute("data-required")),this._requiredFields.forEach(e=>{e.addEventListener("change",e=>isFieldEmpty(e))})}static _studentCardSubscribe(){const e=document.querySelector(".student-card__show-btn-save"),l=document.querySelector("#group"),t=document.querySelector("#course"),a=document.querySelector("#degree"),s=document.querySelector("#speciality");e.addEventListener("click",()=>{!async function(){if(validateFields(StudentCardAddPage._requiredFields)){let t={};StudentCardAddPage._dataObjKeyFields.forEach(e=>{t[e.getAttribute("data-obj-key")]=e.value}),console.log(t),apiHelper.fetchStudentCardSave(t)}}()}),t.addEventListener("change",()=>{!async function(e){if(""!=e){const t=await getGroupsAsOptions(e);l.innerHTML=t.join(""),l.classList.remove("-hasValue")}}(t.value)}),a.addEventListener("change",()=>{!async function(e){let t=await getSpecialitiesAsOptions(e);s.innerHTML=t.join(""),s.classList.remove("-hasValue")}(degree.value)})}}StudentCardAddPage.is="test";class StudentCardShowPage extends Page{constructor(){super()}static _dataObjKeyFields=null;static get is(){return"student-card--show-page"}static get page(){return`
        ...
        `}static init(){this._initDataObjKeyFields(),this._studentCardSubscribe(),subscribeFormElements()}static _initDataObjKeyFields(){this._dataObjKeyFields=content.querySelectorAll("[data-obj-key]"),this._requiredFields=[...this._dataObjKeyFields].filter(e=>e.hasAttribute("data-required")),this._requiredFields.forEach(e=>{e.addEventListener("change",e=>isFieldEmpty(e))})}static _studentCardSubscribe(){const e=document.querySelector(".student-card__show-btn-find"),t=document.querySelector(".student-card__show-btn-edit"),l=document.querySelector(".student-card__show-btn-save"),a=document.querySelector(".student-card__show-transfer-history-container"),s=document.querySelector("#speciality"),i=document.querySelector("#course"),o=document.querySelector("#group"),r=document.querySelector("#year"),n=document.querySelector("#student");let d=[i,r,o,n];e.addEventListener("click",()=>{validateFields(d)&&async function(t){if(""!=t){const l=await apiHelper.fetchGetStudent(t);StudentCardShowPage._dataObjKeyFields.forEach(e=>{e.value=l[e.getAttribute("data-obj-key")],e.value&&e.classList.add("-hasValue")});let e=document.querySelector(".student-card__show-transfer-history-container");e.innerHTML=getStudentTransferHistoryRows(l.transferHistory)}}(n.value)}),t.addEventListener("click",()=>{this._dataObjKeyFields.forEach(e=>{e.disabled=!e.disabled});let e=a.querySelectorAll("[data-history-key]");console.log(e),e.forEach(e=>{e.disabled=!e.disabled})}),l.addEventListener("click",()=>{validateFields(StudentCardShowPage._requiredFields)&&async function(e){let t={};StudentCardShowPage._dataObjKeyFields.forEach(e=>{t[e.getAttribute("data-obj-key")]=""!=e.value?e.value:void 0});var l=a.querySelectorAll(".student-card__show-transfer-history-row");t.transferHistory=[...l].map(e=>{let t=e.querySelectorAll("[data-history-key]"),l={};return t.forEach(e=>{l[e.getAttribute("data-history-key")]=""!=e.value?e.value:void 0}),l}),apiHelper.fetchStudentCardUpdate(e,t)}(n.value)}),i.addEventListener("change",e=>{isFieldEmpty(e)||async function(e){if(""!=e){const t=await getGroupsAsOptions(e);o.innerHTML=t.join(""),o.classList.remove("-hasValue"),n.value="",n.innerHTML=optionDefault,n.classList.remove("-hasValue")}}(i.value)}),o.addEventListener("change",e=>{isFieldEmpty(e)||c(o.value,r.value)}),r.addEventListener("change",e=>{isFieldEmpty(e)||c(o.value,r.value)}),n.addEventListener("change",e=>{isFieldEmpty(e)});async function c(t,l=null){if(""!=t&&""!=l){let e=await getStudentsAsOptions(t,l);n.innerHTML=e.join(""),n.classList.remove("-hasValue")}}window.onload=async function(){let e=await getSpecialitiesAsOptions(0);s.innerHTML=e.join(""),s.classList.remove("-hasValue")}()}}class CurriculumAddPage extends Page{constructor(){super()}static get is(){return"curriculum--add-page"}static get page(){return`
        ...
        `}static init(){this._groupCardSubscribe(),subscribeFormElements()}static buttons={remove:"<i class='remove fas fa-minus-square' onClick='CurriculumAddPage.removeGroupFromList(this)'></i>"};static removeGroupFromList(e){const t=e.closest(".group__item");t.remove()}static _groupCardSubscribe(){const l=document.querySelector(".curriculum__find-groups"),e=document.querySelector(".curriculum__table"),t=document.querySelector(".curriculum__table-row-last"),a=document.querySelector("#add-row-btn"),i=document.querySelector("#year"),o=document.querySelector("#plan-name"),s=document.querySelector("#course"),r=document.querySelector("#group"),n=document.querySelector(".curriculum-group__btn-add"),d=document.querySelector(".curriculum__info-add-btn");i.addEventListener("change",e=>{isFieldEmpty(e)}),r.addEventListener("change",e=>{isFieldEmpty(e)}),s.addEventListener("change",e=>{isFieldEmpty(e)||async function(e){if(""!=e){const t=await getGroupsAsOptions(e);r.innerHTML=t.join(""),r.classList.remove("-hasValue")}}(s.value)}),o.addEventListener("change",e=>{isFieldEmpty(e)}),n.addEventListener("click",e=>{var t;!validateFields([s,r],!0)||(t=getGroupItemWithButton(r.options[group.selectedIndex].text,r.value,CurriculumAddPage.buttons.remove))&&l.insertAdjacentHTML("beforeend",t)}),d.addEventListener("click",()=>{if(validateFields([i,o],!0)){let a={subjectInfo:[]},s=e.querySelectorAll(".curriculum__table-row-data");for(let e=0;e<s.length;e++){let t=s[e].querySelectorAll("[data-table-key]"),l={};for(let e=0;e<t.length;e++)"SELECT"!=t[e].tagName&&"INPUT"!=t[e].tagName||""!=t[e].value?l[t[e].getAttribute("data-table-key")]=t[e].value:l[t[e].getAttribute("data-table-key")]=null;a.subjectInfo.push(l)}a.name=o.value,a.year=i.value,a.groupIds=[...l.children].map(e=>+e.getAttribute("data-group-id")).filter((e,t,l)=>l.indexOf(e)===t),async function(e){apiHelper.fetchCreateYearPlan(e)}(a)}});a.addEventListener("click",()=>{t.insertAdjacentHTML("beforebegin",addCurriculumRow())})}}class CurriculumShowPage extends Page{constructor(){super()}static get is(){return"curriculum--show-page"}static get page(){return`
            <div class="curriculum__grid">
        ...
        `}static init(){this._subscribePageElements(),subscribeFormElements()}static _subscribePageElements(){const i=document.querySelector(".curriculum__table"),a=document.querySelector(".curriculum__table-row-last"),e=document.querySelector("#add-row-btn"),s=document.querySelector("#plan-id"),o=document.querySelector("#plan-name"),t=document.querySelector("#year"),l=document.querySelector("#course"),r=document.querySelector("#group"),n=[r,l,t],d=document.querySelector(".curriculum__btn-find"),c=document.querySelector(".curriculum-info__btn-edit"),m=document.querySelector(".curriculum-info__btn-save");t.addEventListener("change",e=>{isFieldEmpty(e)||u(l.value,t.value)}),l.addEventListener("change",e=>{isFieldEmpty(e)||u(l.value,t.value)}),r.addEventListener("change",e=>{isFieldEmpty(e)}),d.addEventListener("click",e=>{validateFields(n)&&async function(e,t){if(""!=e&&""!=t){!function(){var e=i.querySelectorAll(".curriculum__table-row-data");for(var t of e)t.remove()}();const l=await apiHelper.fetchGetYearPlan(e,t);console.log(l),s.value=l.id,s.classList.add("-hasValue"),o.value=l.name,o.classList.add("-hasValue"),l.subjectInfo.forEach(e=>{a.insertAdjacentHTML("beforebegin",addCurriculumRow(e))})}}(r.value,t.value)}),c.addEventListener("click",()=>{let e=i.querySelectorAll("[data-is-disabled]");e.forEach(e=>{e.disabled=!e.disabled})}),m.addEventListener("click",()=>{!async function(e){if(""!=e){let a={id:e,name:o.value,year:t.value,groupdIds:[r.value],subjectInfo:[]},s=i.querySelectorAll(".curriculum__table-row-data");for(let e=0;e<s.length;e++){let t=s[e].querySelectorAll("[data-table-key]"),l={};for(let e=0;e<t.length;e++)"SELECT"!=t[e].tagName&&"INPUT"!=t[e].tagName||""!=t[e].value?l[t[e].getAttribute("data-table-key")]=t[e].value:l[t[e].getAttribute("data-table-key")]=null;a.subjectInfo.push(l)}apiHelper.fetchUpdateYearPlan(e,a)}}(s.value)});async function u(e,t){if(""!=e&&""!=t){const l=await getGroupsAsOptions(e,t);r.innerHTML=l.join(""),r.classList.remove("-hasValue")}}e.addEventListener("click",()=>{e.hasAttribute("disabled")||a.insertAdjacentHTML("beforebegin",addCurriculumRow())})}}class GroupAddPage extends Page{constructor(){super()}static get is(){return"group--add-page"}static get page(){return`
        ...
        `}static init(){this._groupCardSubscribe(),subscribeFormElements()}static _groupCardSubscribe(){const e=document.querySelector(".group__add__btn-save"),a=document.querySelector("#name");e.addEventListener("click",()=>{!async function(e){var t=document.querySelectorAll(".form-has-error");t.length?console.log("You should fix errors on the page!"):a.value?apiHelper.fetchCreateGroup(e):a.closest(".form-element").classList.add("form-has-error")}(a.value)}),a.addEventListener("change",e=>{let t=a.value,l=e.target.closest(".form-element");l.classList[0<t.length&&2!=t.split("-").length?"add":"remove"]("form-has-error")})}}class GroupActualizePage extends Page{constructor(){super()}static get is(){return"group--actualize-page"}static get page(){return`
        ...
        `}static init(){this._groupCardSubscribe(),subscribeFormElements()}static _groupCardSubscribe(){const s=document.querySelector(".transfer__students-list"),t=document.querySelector("#course"),i=document.querySelector("#group"),e=document.querySelector("#year"),l=[t,e,i],a=document.querySelector("#btn-find"),o=document.querySelector("#btn-transfer");t.addEventListener("change",e=>{isFieldEmpty(e)||async function(e){if(""!=e){const t=await getGroupsAsOptions(e);i.innerHTML=t.join(""),i.classList.remove("-hasValue")}}(t.value)}),i.addEventListener("change",e=>{isFieldEmpty(e)}),e.addEventListener("change",e=>{isFieldEmpty(e)}),a.addEventListener("click",()=>{validateFields(l)&&async function(e,t){if(""!=e&&""!=t){s.innerHTML="";const l=await apiHelper.fetchGetStudentsForTransfer(e,t);l.forEach(e=>{s.insertAdjacentHTML("beforeend",getTransferStudentRow(e))})}}(i.value,e.value)}),o.addEventListener("click",()=>{!async function(){let a=[];const e=[...s.querySelectorAll(".transfer__student-row.active")];e.forEach(e=>{const t=e.querySelectorAll("[data-transfer-key]");let l={groupId:i.value};t.forEach(e=>{l[e.getAttribute("data-transfer-key")]="SPAN"==e.tagName?e.innerText:e.value}),a.push(l)}),apiHelper.fetchUpdateTransferStudents(a)}()})}}class CreateReportPage extends Page{constructor(){super()}static get is(){return"create-report--page"}static get page(){return`
        ...
        `}static init(){this._groupCardSubscribe(),subscribeFormElements()}static _groupCardSubscribe(){const t=document.querySelector("#course"),l=document.querySelector("#year"),a=document.querySelector("#group"),s=document.querySelector("#student"),e=document.querySelector(".create-report__create-btn");let i=[t,l,a,s];t.addEventListener("change",e=>{isFieldEmpty(e)||o(t.value,l.value)}),l.addEventListener("change",e=>{isFieldEmpty(e)||o(t.value,l.value)}),a.addEventListener("change",e=>{isFieldEmpty(e)||async function(e){const t=await getStudentsAsOptions(e);s.innerHTML=t.join(""),s.classList.remove("-hasValue")}(a.value)}),s.addEventListener("change",e=>{isFieldEmpty(e)}),e.addEventListener("click",()=>{validateFields(i)&&async function(e){toggleMask(),await apiHelper.fetchCreateReport(e),toggleMask()}(s.value)});async function o(e,t){const l=await getGroupsAsOptions(e,t);a.innerHTML=l.join(""),a.classList.remove("-hasValue"),s.value="",s.innerHTML=optionDefault,s.classList.remove("-hasValue")}}}class ProgressPage extends Page{constructor(){super()}static get is(){return"progress--page"}static get page(){return`
        ...
        `}static init(){this._subscribePageElements(),subscribeFormElements()}static _subscribePageElements(){const t=document.querySelector("#year"),l=document.querySelector("#course"),a=document.querySelector("#group"),e=document.querySelector("#semestr"),s=[t,l,a,e],i=document.querySelector(".progress__table"),o=document.querySelector(".progress__row-cell-subjects"),r=document.querySelector(".progress__btn-find"),n=document.querySelector(".progress__btn-edit"),d=document.querySelector(".progress__btn-save");t.addEventListener("change",e=>{isFieldEmpty(e)||c(l.value,t.value)}),l.addEventListener("change",e=>{isFieldEmpty(e)||c(l.value,t.value)}),a.addEventListener("change",e=>{isFieldEmpty(e)}),e.addEventListener("change",e=>{isFieldEmpty(e)}),r.addEventListener("click",()=>{validateFields(s)&&async function(e,t,l){if(""!=e&&""!=t&&""!=l){!function(){let l=i.querySelectorAll(".progress__row-cell-subject, .progress__row-cell-subject-with-cw");for(let e=0,t=l.length;e<t;e++)l[e].remove();let a=i.querySelectorAll(".progress__row-student");for(let e=0,t=a.length;e<t;e++)a[e].remove()}();const a=await apiHelper.fetchGetProgress(e,t,l);a.subjects.forEach(e=>{o.insertAdjacentHTML("beforeend",getProgressSubjectCell(e))}),a.students.forEach(e=>{i.insertAdjacentHTML("beforeend",getProgressStudentRow(e,a.subjects))})}}(t.value,a.value,e.value)}),n.addEventListener("click",()=>{let e=i.querySelectorAll(".progress__row-cell-marks input");e.forEach(e=>{e.disabled=!e.disabled})}),d.addEventListener("click",()=>{let l=[],e=i.querySelectorAll(".progress__row-student");e.forEach(e=>{let t={id:e.getAttribute("data-student-id"),name:e.querySelector(".progress__row-cell-student span")?.innerText};t.subjects=[...e.querySelectorAll(".progress__row-cell-mark, .progress__row-cell-mark-with-cw")].map(e=>{let t=e.querySelector(".progress__row-cell-mark-value");var l=e.querySelector(".progress__row-cell-mark-date"),e=e.querySelector(".progress__row-cell-cw-value");let a={id:+t.getAttribute("data-subject-id"),mark:""==t.value?null:t.value,modifiedOn:""==l.value?null:l.value};return e&&(a.taskMark=""==e.value?null:e.value),a}),l.push(t)}),async function(e){apiHelper.fetchUpdateProgress(e)}(l)});async function c(e,t){if(""!=e&&""!=t){const l=await getGroupsAsOptions(e,t);a.innerHTML=l.join(""),a.classList.remove("-hasValue")}}}}content||console.log("Tag with class'main__content' not found!");const mapper={"main--page":MainPage,"student-card--add-page":StudentCardAddPage,"student-card--show-page":StudentCardShowPage,"curriculum--show-page":CurriculumShowPage,"curriculum--add-page":CurriculumAddPage,"transfer--page":TransferPage,"progress--page":ProgressPage,"create-report--page":CreateReportPage,"group--add-page":GroupAddPage,"group--actualize-page":GroupActualizePage};function setPageByAttribute(t){if(t in mapper){let e=mapper[t];content.innerHTML=e.page,e.init()}else console.log(`Tag with attribute data-page='${t}' not found!`)}function init(){initSidebar(),initMainPage()}init();