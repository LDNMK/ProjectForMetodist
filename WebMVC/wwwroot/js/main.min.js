function initSidebar(){let e=document.body.querySelectorAll("a[data-page*='-page'");e.forEach(e=>{e.addEventListener("click",e=>{setPageByAttribute(e.target.getAttribute("data-page"))})})}function subscribeFormElements(){let e=document.body.querySelectorAll(".form-element-field");e.forEach(e=>e.addEventListener("change",e=>{let t=e.target;t.classList[t.value?"add":"remove"]("-hasValue")}))}const optionDefault='<option class="form-select-placeholder" value="" disabled selected></option>';class Page{constructor(){}static getPage(){return"not implemented"}static init(){console.log("not initialized")}static is(){return"Page"}static _idsToClear(){return[]}}class MainPage extends Page{constructor(){super()}static getPage(){return`
            <h1>Main page</h1>
        `}static subscribe(){super.subscribe()}static is(){return"main--page"}}function initMainPage(){setPageByAttribute(MainPage.is())}class StudentCardShowPage extends Page{constructor(){super()}static _dataObjKeyFields=null;static is(){return"student-card--show-page"}static getPage(){return`
        ...
        `}static init(){this._initDataObjKeyFields(),this._studentCardShowSubscribe(),subscribeFormElements()}static _initDataObjKeyFields(){this._dataObjKeyFields=content.querySelectorAll("[data-obj-key]")}static _studentCardShowSubscribe(){const e=document.querySelector(".student-card__show-btn-clear"),t=document.querySelector(".student-card__show-btn-find"),l=document.querySelector(".student-card__show-btn-edit"),a=document.querySelector(".student-card__show-btn-save"),s=document.querySelector(".student-card__show-average-score-btn"),o=document.querySelector("#group"),n=document.querySelector("#course"),i=document.querySelector("#student"),d=document.querySelector("#year");e.addEventListener("click",()=>{let e=this._idsToClear(),t=document.querySelectorAll(e.join(", "));t.forEach(e=>{e.classList.remove("-hasValue"),e.value=""})}),t.addEventListener("click",()=>{!async function(e){if(""!=e){const t=await fetch(`api/StudentCard/ShowStudentInfo?studentId=${e}`),l=await t.json();StudentCardShowPage._dataObjKeyFields.forEach(e=>{e.value=l[e.getAttribute("data-obj-key")],e.classList.add("-hasValue")}),console.log(l)}}(i.value)}),l.addEventListener("click",()=>{console.log("Edit")}),a.addEventListener("click",()=>{!async function(e){let t={};StudentCardShowPage._dataObjKeyFields.forEach(e=>{t[e.getAttribute("data-obj-key")]=e.value}),console.log(t);await fetch(`api/StudentCard/UpdateStudentCardInfo?studentId=${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)})}(i.value)}),s.addEventListener("click",()=>{console.log("Average score")}),d.addEventListener("change",()=>{r(d.value,n.value)}),n.addEventListener("change",()=>{r(d.value,n.value)}),o.addEventListener("change",()=>{!async function(t){if(""!=t){const l=await fetch(`api/StudentCard/GetListOfStudents?groupId=${t}`),a=await l.json();let e=a.map(e=>`<option value=${e.studentId}>${e.studentName}</option>`);e.push(optionDefault),i.innerHTML=e.join(""),i.classList.remove("-hasValue")}}(o.value)});async function r(t,l){if(""!=t&&""!=l){const a=await fetch(`api/Group/GetListOfGroups?course=${l}&year=${t}`),s=await a.json();console.log(s);let e=s.map(e=>`<option value=${e.groupId}>${e.groupName}</option>`);e.push(optionDefault),o.innerHTML=e.join(""),o.classList.remove("-hasValue")}}d.value=2021,d.classList.add("-hasValue"),n.value="1",n.classList.add("-hasValue");var c=new Event("change");n.dispatchEvent(c)}static _idsToClear(){return["#speciality","#specialization","#year","#course","#group","#student"]}}let content=document.body.querySelector(".main__content");content||console.log("Tag with class'main__content' not found!");const mapper={"main--page":MainPage,"student-card--add-page":()=>"student-card--add-page not implemented","student-card--show-page":StudentCardShowPage,"curriculum--show-page":()=>"curriculum--show-page not implemented","curriculum--add-page":()=>"curriculum--add-page not implemented","transfer--page":()=>"transfer--page not implemented","progress--page":()=>"progress--page not implemented","report--add-page":()=>"report--add-page not implemented","group--add-page":()=>"group--add-page not implemented"};function setPageByAttribute(t){if(t in mapper){let e=mapper[t];content.innerHTML=e.getPage(),e.init()}else console.log(`Tag with attribute data-page='${t}' not found!`)}function init(){initSidebar(),setPageByAttribute(StudentCardShowPage.is())}init();