const msgContainer=document.querySelector(".notification__list"),content=document.body.querySelector(".main__content"),notificationTimeout={success:5e3,error:6e3},notificationTitle={success:"Успішно",error:"Помилка"},notificationMessages={curriculumSave:"Навчальний план був збережений",curriculumSaveError:"Виникла помилка при збереженнні навчального плану",curriculumEditSaveError:"Виникла помилка при збереженнні навчального плану після редагування",curriculumFindError:"Виникла помилка при пошуку навчального плану"};function initSidebar(){let e=document.body.querySelectorAll("a[data-page*='-page'"),t=document.body.querySelectorAll(".sidebar__menu-item");e.forEach(e=>{e.addEventListener("click",e=>{setPageByAttribute(e.target.getAttribute("data-page")),t.forEach(e=>e.classList.remove("active")),e.target.closest(".sidebar__menu-item").classList.add("active")})})}function getGroupItemWithButton(e,t,a){return""==t||""==e||null==a?null:`
    ...
    `}function getProgressStudentRow(s,e){return`
        <div class="progress__row progress__row-student" data-student-id="${s.id}">
            ...
            <ul class="progress__row-cell progress__row-cell-marks">
                ${e.map(t=>{let e=s.subjects.find(e=>e.id==t.id);var a=e?.mark??"",l=e?.modifiedOn?e.modifiedOn.split("T")[0]:"";return console.log(l),`
    ...
    `}function getStudentTransferHistoryRows(e){return`
        ${e.map(e=>`
            ...
            `).join("")}
    `}function getNotificationElement(e){let t=`
        <li class="notification__list-item showNotification ${e.type}">
            <div class="notification__list-item-container">
                ${"success"==e.type?'<i class="success-icon fas fa-check-circle"></i>':'<i class="error-icon fas fa-times-circle"></i>'}
    ...
    `;return createElementFromHTML(t.trim())}function createElementFromHTML(e){var t=document.createElement("div");return t.innerHTML=e.trim(),t.firstChild}function addNotification(e,t){e={title:notificationTitle[e],text:t,type:e,timeout:notificationTimeout[e]};let a=getNotificationElement(e);msgContainer.insertAdjacentElement("beforeend",a),setTimeout(()=>{a.classList.remove("showNotification"),setTimeout(()=>{a.remove()},700)},e.timeout)}function subscribeFormElements(){let e=document.body.querySelectorAll(".form-element-field");e.forEach(e=>e.addEventListener("change",e=>{let t=e.target;t.classList[t.value?"add":"remove"]("-hasValue")}))}const optionDefault='<option class="form-select-placeholder" value="" disabled selected></option>';class Page{constructor(){}static getPage(){return"not implemented"}static init(){console.log("not initialized")}static get is(){return"Page"}static _idsToClear(){return[]}}class MainPage extends Page{constructor(){super()}static get page(){return`
            <h1>Main page</h1>
        ...
        `}static init(){this._initDataObjKeyFields(),this._studentCardSubscribe(),subscribeFormElements()}static _initDataObjKeyFields(){this._dataObjKeyFields=content.querySelectorAll("[data-obj-key]")}static _studentCardSubscribe(){const e=document.querySelector(".student-card__show-btn-save"),s=document.querySelector("#group"),t=document.querySelector("#course"),a=document.querySelector("#degree"),i=document.querySelector("#speciality");e.addEventListener("click",()=>{!async function(){let t={};StudentCardAddPage._dataObjKeyFields.forEach(e=>{t[e.getAttribute("data-obj-key")]=e.value}),console.log(t);const e=await fetch("api/StudentCard/SaveStudentCardInfo",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)});var a;e.ok||(a=await e.json(),console.log(a))}()}),t.addEventListener("change",()=>{!async function(t){if(""!=t){const a=await fetch(`api/Group/GetGroups?course=${t}`),l=await a.json();console.log(l);let e=l.map(e=>`<option value=${e.groupId}>${e.groupName}</option>`);e.push(optionDefault),s.innerHTML=e.join(""),s.classList.remove("-hasValue")}}(t.value)}),a.addEventListener("change",()=>{!async function(e){const t=await fetch(`api/StudentCard/GetSpecialities?degreeId=${e}`),a=await t.json();let l=a.map(e=>`<option value=${e.id}>${e.name}</option>`);l.push(optionDefault),i.innerHTML=l.join(""),i.classList.remove("-hasValue")}(degree.value)})}}StudentCardAddPage.is="test";class StudentCardShowPage extends Page{constructor(){super()}static _dataObjKeyFields=null;static get is(){return"student-card--show-page"}static get page(){return`
        ...
        `}static init(){this._initDataObjKeyFields(),this._studentCardSubscribe(),subscribeFormElements()}static _initDataObjKeyFields(){this._dataObjKeyFields=content.querySelectorAll("[data-obj-key]")}static _studentCardSubscribe(){const e=document.querySelector(".student-card__show-btn-find"),t=document.querySelector(".student-card__show-btn-edit"),a=document.querySelector(".student-card__show-btn-save"),l=document.querySelector(".student-card__show-average-score-btn"),s=document.querySelector(".student-card__show-transfer-history-container"),i=document.querySelector("#group"),o=document.querySelector("#course"),r=document.querySelector("#student"),n=document.querySelector("#speciality"),d=document.querySelector("#select-year"),c=document.querySelector("#year");e.addEventListener("click",()=>{!async function(t){if(""!=t){const a=await fetch(`api/StudentCard/ShowStudentInfo?studentId=${t}`),l=await a.json();StudentCardShowPage._dataObjKeyFields.forEach(e=>{e.value=l[e.getAttribute("data-obj-key")],e.value&&e.classList.add("-hasValue")});let e=document.querySelector(".student-card__show-transfer-history-container");e.innerHTML=getStudentTransferHistoryRows(l.transferHistory),console.log(l)}}(r.value)}),t.addEventListener("click",()=>{this._dataObjKeyFields.forEach(e=>{e.disabled=!e.disabled});let e=s.querySelectorAll("[data-history-key]");console.log(e),e.forEach(e=>{e.disabled=!e.disabled})}),a.addEventListener("click",()=>{!async function(e){let t={};StudentCardShowPage._dataObjKeyFields.forEach(e=>{t[e.getAttribute("data-obj-key")]=""!=e.value?e.value:void 0});var a=s.querySelectorAll(".student-card__show-transfer-history-row");t.transferHistory=[...a].map(e=>{let t=e.querySelectorAll("[data-history-key]"),a={};return t.forEach(e=>{a[e.getAttribute("data-history-key")]=""!=e.value?e.value:void 0}),a}),console.log(t);await fetch(`api/StudentCard/UpdateStudentCardInfo?studentId=${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)})}(r.value)}),l.addEventListener("click",()=>{console.log("Average score")}),d.addEventListener("change",()=>{c.disabled=!d.checked,d.checked||(c.value="",c.classList.remove("-hasValue"),i.dispatchEvent(new Event("change")))}),c.addEventListener("change",()=>{u(i.value,c.value)}),o.addEventListener("change",()=>{!async function(t){if(""!=t){const a=await fetch(`api/Group/GetGroups?course=${t}`),l=await a.json();console.log(l);let e=l.map(e=>`<option value=${e.groupId}>${e.groupName}</option>`);e.push(optionDefault),i.innerHTML=e.join(""),i.classList.remove("-hasValue"),r.value="",r.innerHTML=optionDefault,r.classList.remove("-hasValue")}}(o.value)}),i.addEventListener("change",()=>{d.checked&&!c.value?console.log("Error: year wasn't selected"):d.checked&&c.value?u(i.value,c.value):u(i.value)});async function u(a,l=null){if(""!=a&&""!=l){let e=`api/StudentCard/GetStudents?groupId=${a}`;l&&(e+=`&year=${l}`);const s=await fetch(e),i=await s.json();console.log(i);let t=i.map(e=>`<option value=${e.studentId}>${e.studentName}</option>`);t.push(optionDefault),r.innerHTML=t.join(""),r.classList.remove("-hasValue")}}window.onload=async function(){const e=await fetch("api/StudentCard/GetSpecialities"),t=await e.json();let a=t.map(e=>`<option value=${e.id}>${e.name}</option>`);a.push(optionDefault),n.innerHTML=a.join(""),n.classList.remove("-hasValue")}()}static _idsToClear(){return["#speciality","#specialization","#year","#course","#group","#student"]}}class CurriculumAddPage extends Page{constructor(){super()}static get is(){return"curriculum--add-page"}static _counter=1;static get page(){return`
        ...
        `}static init(){this._groupCardSubscribe(),subscribeFormElements()}static buttons={remove:"<i class='remove fas fa-minus-square' onClick='CurriculumAddPage.removeGroupFromList(this)'></i>"};static removeGroupFromList(e){const t=e.closest(".group__item");t.remove()}static _groupCardSubscribe(){const t=document.querySelector(".curriculum__find-groups"),e=document.querySelector(".curriculum__table"),a=document.querySelector(".curriculum__table-row-last"),l=document.querySelector("#add-row-btn"),i=document.querySelector("#year"),o=document.querySelector("#plan-name"),s=document.querySelector("#course"),r=document.querySelector("#group"),n=document.querySelector(".curriculum-group__btn-add"),d=document.querySelector(".curriculum__info-add-btn");i.addEventListener("change",()=>{console.log("year")}),s.addEventListener("change",()=>{!async function(t){if(""!=t){const a=await fetch(`api/Group/GetGroups?course=${t}`),l=await a.json();let e=l.map(e=>`<option value=${e.groupId}>${e.groupName}</option>`);e.push(optionDefault),r.innerHTML=e.join(""),r.classList.remove("-hasValue")}}(s.value)}),n.addEventListener("click",()=>{var e=getGroupItemWithButton(r.options[group.selectedIndex].text,r.value,CurriculumAddPage.buttons.remove);e&&t.insertAdjacentHTML("beforeend",e)}),d.addEventListener("click",()=>{if(""==i||""==o)return alert("bad");let l={subjectInfo:[]},s=e.querySelectorAll(".curriculum__table-row-data");for(let e=0;e<s.length;e++){let t=s[e].querySelectorAll("[data-table-key]"),a={};for(let e=0;e<t.length;e++)"SELECT"!=t[e].tagName&&"INPUT"!=t[e].tagName||""!=t[e].value?a[t[e].getAttribute("data-table-key")]=t[e].value:a[t[e].getAttribute("data-table-key")]=null;l.subjectInfo.push(a)}l.name=o.value,l.year=i.value,l.groupIds=[...t.children].map(e=>+e.getAttribute("data-group-id")).filter((e,t,a)=>a.indexOf(e)===t),console.log(l),async function(e){e=await fetch("api/YearPlan/AddYearPlan",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)});console.log(e)}(l)});l.addEventListener("click",()=>{a.insertAdjacentHTML("beforebegin",addCurriculumRow())})}}class CurriculumShowPage extends Page{constructor(){super()}static get is(){return"curriculum--show-page"}static get page(){return`
            <div class="curriculum__grid">
        ...
        `}static init(){this._subscribePageElements(),subscribeFormElements()}static _subscribePageElements(){const t=document.querySelector(".curriculum__table"),s=document.querySelector(".curriculum__table-row-last"),e=document.querySelector("#add-row-btn"),i=document.querySelector("#plan-id"),o=document.querySelector("#plan-name"),a=document.querySelector("#year"),l=document.querySelector("#course"),r=document.querySelector("#group"),n=document.querySelector(".curriculum__btn-find"),d=document.querySelector(".curriculum-info__btn-edit"),c=document.querySelector(".curriculum-info__btn-save");a.addEventListener("change",()=>{u(l.value,a.value)}),l.addEventListener("change",()=>{u(l.value,a.value)}),n.addEventListener("click",()=>{!async function(e,t){if(""!=e){const a=await fetch(`api/YearPlan/GetYearPlanByGroup?groupId=${e}&year=${t}`);if(console.log(a),a.ok){const l=await a.json();console.log(l),i.value=l.id,i.classList.add("-hasValue"),o.value=l.name,o.classList.add("-hasValue"),l.subjectInfo.forEach(e=>{s.insertAdjacentHTML("beforebegin",addCurriculumRow(e))})}else{t=await a.json();console.log(t)}}}(r.value,a.value)}),d.addEventListener("click",()=>{let e=t.querySelectorAll("[data-is-disabled]");e.forEach(e=>{e.disabled=!e.disabled})}),c.addEventListener("click",()=>{!async function(e){if(""!=e){let l={id:e,name:o.value,year:a.value,groupdIds:[r.value],subjectInfo:[]},s=t.querySelectorAll(".curriculum__table-row-data");for(let e=0;e<s.length;e++){let t=s[e].querySelectorAll("[data-table-key]"),a={};for(let e=0;e<t.length;e++)"SELECT"!=t[e].tagName&&"INPUT"!=t[e].tagName||""!=t[e].value?a[t[e].getAttribute("data-table-key")]=t[e].value:a[t[e].getAttribute("data-table-key")]=null;l.subjectInfo.push(a)}console.log(l);e=await fetch(`api/YearPlan/UpdateYearPlan?yearPlanId=${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(l)});console.log(e),addNotification("".notificationMessages.curriculumEditSaveError)}}(i.value)});async function u(t,a){if(""!=t){t=`api/Group/GetGroups?course=${t}`;t+=a?`&year=${a}`:"";const l=await fetch(t),s=await l.json();let e=s.map(e=>`<option value=${e.groupId}>${e.groupName}</option>`);e.push(optionDefault),r.innerHTML=e.join(""),r.classList.remove("-hasValue")}}e.addEventListener("click",()=>{e.hasAttribute("disabled")||s.insertAdjacentHTML("beforebegin",addCurriculumRow())})}}class GroupAddPage extends Page{constructor(){super()}static get is(){return"group--add-page"}static get page(){return`
        ...
        `}static init(){this._groupCardSubscribe(),subscribeFormElements()}static _groupCardSubscribe(){const e=document.querySelector(".group__add__btn-save"),l=document.querySelector("#name");e.addEventListener("click",()=>{!async function(e){var t=document.querySelectorAll(".form-has-error");if(t.length)console.log("You should fix errors on the page!");else if(l.value){const a=await fetch(`api/Group/CreateGroup?groupName=${e}`,{method:"POST"});a.ok||(e=await a.json(),console.log(e))}else l.closest(".form-element").classList.add("form-has-error")}(l.value)}),l.addEventListener("change",e=>{let t=l.value,a=e.target.closest(".form-element");a.classList[0<t.length&&2!=t.split("-").length?"add":"remove"]("form-has-error")})}}class GroupActualizePage extends Page{constructor(){super()}static get is(){return"group--actualize-page"}static get page(){return`
        ...
        `}static init(){this._groupCardSubscribe(),subscribeFormElements()}static _groupCardSubscribe(){const s=document.querySelector(".transfer__students-list"),e=document.querySelector("#year"),t=document.querySelector("#course"),i=document.querySelector("#group"),a=document.querySelector("#btn-find"),l=document.querySelector("#btn-transfer");t.addEventListener("change",()=>{!async function(t){if(""!=t){const a=await fetch(`api/Group/GetGroups?course=${t}`),l=await a.json();let e=l.map(e=>`<option value=${e.groupId}>${e.groupName}</option>`);e.push(optionDefault),i.innerHTML=e.join(""),i.classList.remove("-hasValue")}else console.log("Course is empty")}(t.value)}),a.addEventListener("click",()=>{!async function(e,t){if(""!=e&&""!=t){const a=await fetch(`api/Transfer/GetStudentsForTransfer?groupId=${e}&year=${t}`),l=await a.json();s.innerHTML="",s.insertAdjacentHTML("beforeend",getTransferStudentHead()),l.forEach(e=>{s.insertAdjacentHTML("beforeend",getTransferStudentRow(e))})}else s.innerHTML=""}(i.value,e.value)}),l.addEventListener("click",()=>{!async function(){let l=[];const e=[...s.querySelectorAll(".transfer__student-row")];e.forEach(e=>{const t=e.querySelectorAll("[data-transfer-key]");let a={groupId:i.value};t.forEach(e=>{a[e.getAttribute("data-transfer-key")]="SPAN"==e.tagName?e.innerText:e.value}),l.push(a)}),console.log(l);await fetch("api/Transfer/TransferStudents",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(l)})}()})}}class CreateReportPage extends Page{constructor(){super()}static get is(){return"create-report--page"}static get page(){return`
        ...
        `}static init(){this._groupCardSubscribe(),subscribeFormElements()}static _groupCardSubscribe(){const e=document.querySelector("#course"),t=document.querySelector("#year"),i=document.querySelector("#group"),o=document.querySelector("#student"),a=document.querySelector(".create-report__create-btn");e.addEventListener("change",()=>{l(e.value,t.value)}),t.addEventListener("change",()=>{l(e.value,t.value)}),i.addEventListener("change",()=>{!async function(t,a){if(""!=t){""==a&&alert("write year");const l=await fetch(`api/StudentCard/GetStudents?groupId=${t}&year=${a}`),s=await l.json();let e=s.map(e=>`<option value=${e.studentId}>${e.studentName}</option>`);e.push(optionDefault),o.innerHTML=e.join(""),o.classList.remove("-hasValue")}}(i.value,t.value)}),a.addEventListener("click",()=>{!async function(e){if(""!=e){const t=await fetch(`api/Report/CreateReport?studentId=${e}`);await t.json()}}(o.value)});async function l(t,a){if(""!=t&&""!=a){const l=await fetch(`api/Group/GetGroups?course=${t}&year=${a}`),s=await l.json();console.log(s);let e=s.map(e=>`<option value=${e.groupId}>${e.groupName}</option>`);e.push(optionDefault),i.innerHTML=e.join(""),i.classList.remove("-hasValue"),o.value="",o.innerHTML=optionDefault,o.classList.remove("-hasValue")}}}}class ProgressPage extends Page{constructor(){super()}static get is(){return"progress--page"}static get page(){return`
        ...
        `}static init(){this._subscribePageElements(),subscribeFormElements()}static _subscribePageElements(){const e=document.querySelector("#year"),t=document.querySelector("#course"),i=document.querySelector("#group"),a=document.querySelector("#semestr"),o=document.querySelector(".progress__table"),r=document.querySelector(".progress__row-cell-subjects"),l=document.querySelector(".progress__btn-find"),s=document.querySelector(".progress__btn-edit"),n=document.querySelector(".progress__btn-save");e.addEventListener("change",()=>{}),t.addEventListener("change",()=>{!async function(t,a){if(""!=t){t=`api/Group/GetGroups?course=${t}`;t+=a?`&year=${a}`:"";const l=await fetch(t),s=await l.json();let e=s.map(e=>`<option value=${e.groupId}>${e.groupName}</option>`);e.push(optionDefault),i.innerHTML=e.join(""),i.classList.remove("-hasValue")}}(t.value)}),l.addEventListener("click",()=>{!async function(e,t,a){if(""!=e&&""!=t&&""!=a){!function(){let a=o.querySelectorAll(".progress__row-cell-subject");for(let e=0,t=a.length;e<t;e++)a[e].remove();let l=o.querySelectorAll(".progress__row-student");for(let e=0,t=l.length;e<t;e++)l[e].remove()}();a=`api/Progress/GetProgress?year=${e}&groupId=${t}&semesterId=${a}`;console.log(a);const l=await fetch(a),s=await l.json();s.subjects.forEach(e=>{r.insertAdjacentHTML("beforeend",getProgressSubjectCell(e))}),s.students.forEach(e=>{o.insertAdjacentHTML("beforeend",getProgressStudentRow(e,s.subjects))}),console.log(s)}}(e.value,i.value,a.value)}),s.addEventListener("click",()=>{let e=o.querySelectorAll(".progress__row-cell-marks input");e.forEach(e=>{e.disabled=!e.disabled})}),n.addEventListener("click",()=>{let a=[],e=o.querySelectorAll(".progress__row-student");e.forEach(e=>{let t={id:e.getAttribute("data-student-id"),name:e.querySelector(".progress__row-cell-student span")?.innerText};t.subjects=[...e.querySelectorAll(".progress__row-cell-mark")].map(e=>{let t=e.querySelector(".progress__row-cell-mark-value");e=e.querySelector(".progress__row-cell-mark-date");return{id:+t.getAttribute("data-subject-id"),mark:""==t.value?null:t.value,modifiedOn:""==e.value?null:e.value}}),a.push(t)}),async function(e){console.log(e);e=await fetch("api/Progress/UpdateProgress",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)});console.log(e)}(a)})}}content||console.log("Tag with class'main__content' not found!");const mapper={"main--page":MainPage,"student-card--add-page":StudentCardAddPage,"student-card--show-page":StudentCardShowPage,"curriculum--show-page":CurriculumShowPage,"curriculum--add-page":CurriculumAddPage,"transfer--page":TransferPage,"progress--page":ProgressPage,"create-report--page":CreateReportPage,"group--add-page":GroupAddPage,"group--actualize-page":GroupActualizePage};function setPageByAttribute(t){if(t in mapper){let e=mapper[t];content.innerHTML=e.page,e.init()}else console.log(`Tag with attribute data-page='${t}' not found!`)}function init(){initSidebar(),setPageByAttribute(CurriculumShowPage.is)}init();