const msgContainer=document.querySelector(".notification__list"),content=document.body.querySelector(".main__content"),notificationTimeout={success:5e3,warning:6e3,error:6e3},notificationTitle={success:"Успішно",warning:"Попередження",error:"Помилка"},notificationIcons={success:'<i class="success-icon fas fa-check-circle"></i>',warning:'<i class="warning-icon fas fa-exclamation-circle"></i>',error:'<i class="error-icon fas fa-times-circle"></i>'};class ApiHelper{async _callApi(e,t){const a=await fetch(e,t);t=await a.json();return console.log("after json"),console.log(t),t.notificationType?(console.log("in if"),NotificationHelper.addNotification(t.notificationType,t.notificationText),t.data):(console.log("before last return"),console.log(t),t)}async fetchCreateGroup(e){await this._callApi(`api/Group/CreateGroup?groupName=${e}`,{method:"POST"})}async fetchGetGroups(e,t){return await this._callApi(`api/Group/GetGroups?course=${e}${t?`&year=${t}`:""}`)}async fetchGetStudents(e,t){return await this._callApi(`api/StudentCard/GetStudents?groupId=${e}&year=${t}`)}async fetchCreateReport(e){await this._callApi(`api/Report/CreateReport?studentId=${e}`)}async fetchCreateYearPlan(e){await this._callApi("api/YearPlan/AddYearPlan",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)})}async fetchUpdateYearPlan(e,t){await this._callApi(`api/YearPlan/UpdateYearPlan?yearPlanId=${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)})}async fetchGetYearPlan(e,t){return await this._callApi(`api/YearPlan/GetYearPlanByGroup?groupId=${e}&year=${t}`)}}class NotificationHelper{static addNotification(e,t){e={title:notificationTitle[e],text:t,type:e,timeout:notificationTimeout[e]};let a=this._getNotificationElement(e);msgContainer.insertAdjacentElement("beforeend",a),setTimeout(()=>{a.classList.remove("showNotification"),setTimeout(()=>{a.remove()},700)},e.timeout)}static _getNotificationElement(e){let t=`
            <li class="notification__list-item showNotification ${e.type}">
                <div class="notification__list-item-container">
                    ${notificationIcons[e.type]}
    ...
    `}function subscribeFormElements(){let e=document.body.querySelectorAll(".form-element-field");e.forEach(e=>e.addEventListener("change",e=>{let t=e.target;t.classList[t.value?"add":"remove"]("-hasValue")}))}const optionDefault='<option class="form-select-placeholder" value="" disabled selected></option>';class Page{constructor(){}static getPage(){return"not implemented"}static init(){console.log("not initialized")}static get is(){return"Page"}static _idsToClear(){return[]}}class MainPage extends Page{constructor(){super()}static get page(){return`
        ...
        `}static init(){this._subscribePageElements(),subscribeFormElements()}static _subscribePageElements(){const t=document.querySelector(".curriculum__table"),l=document.querySelector(".curriculum__table-row-last"),e=document.querySelector("#add-row-btn"),s=document.querySelector("#plan-id"),i=document.querySelector("#plan-name"),a=document.querySelector("#year"),o=document.querySelector("#course"),r=document.querySelector("#group"),n=document.querySelector(".curriculum__btn-find"),c=document.querySelector(".curriculum-info__btn-edit"),d=document.querySelector(".curriculum-info__btn-save");a.addEventListener("change",()=>{u(o.value,a.value)}),o.addEventListener("change",()=>{u(o.value,a.value)}),n.addEventListener("click",()=>{!async function(e,t){if(""!=e&&""!=t){const a=await apiHelper.fetchGetYearPlan(e,t);console.log("in curriculum page"),console.log(a),s.value=a.id,s.classList.add("-hasValue"),i.value=a.name,i.classList.add("-hasValue"),a.subjectInfo.forEach(e=>{l.insertAdjacentHTML("beforebegin",addCurriculumRow(e))})}}(r.value,a.value)}),c.addEventListener("click",()=>{let e=t.querySelectorAll("[data-is-disabled]");e.forEach(e=>{e.disabled=!e.disabled})}),d.addEventListener("click",()=>{!async function(e){if(""!=e){let l={id:e,name:i.value,year:a.value,groupdIds:[r.value],subjectInfo:[]},s=t.querySelectorAll(".curriculum__table-row-data");for(let e=0;e<s.length;e++){let t=s[e].querySelectorAll("[data-table-key]"),a={};for(let e=0;e<t.length;e++)"SELECT"!=t[e].tagName&&"INPUT"!=t[e].tagName||""!=t[e].value?a[t[e].getAttribute("data-table-key")]=t[e].value:a[t[e].getAttribute("data-table-key")]=null;l.subjectInfo.push(a)}apiHelper.fetchUpdateYearPlan(e,l)}}(s.value)});async function u(e,t){if(""!=e&&""!=t){const a=await getGroupsAsOptions(e,t);r.innerHTML=a.join(""),r.classList.remove("-hasValue")}}e.addEventListener("click",()=>{e.hasAttribute("disabled")||l.insertAdjacentHTML("beforebegin",addCurriculumRow())})}}class GroupAddPage extends Page{constructor(){super()}static get is(){return"group--add-page"}static get page(){return`
