let tag=document.body.querySelector(".main__content");tag||console.log("Tag with class'main__content' not found!");const mapper={mainPage:()=>"not implemented",studentCardAddPage:()=>"not implemented",studentCardShowPage:getStudentCardShowPage};function setPageByAttribute(e){console.log(e),e in mapper?(tag.innerHTML=mapper[e](),subscribeFormElements()):console.log(`Tag with attribute data-page='${e}' not found!`)}function initSidebar(){let e=document.body.querySelectorAll("a[data-page*='Page'");e.forEach(e=>{e.addEventListener("click",e=>{setPageByAttribute(e.target.getAttribute("data-page"))})})}function subscribeFormElements(){let e=document.body.querySelectorAll(".form-element-field");e.forEach(e=>e.addEventListener("change",e=>{let l=e.target;l.classList[l.value?"add":"remove"]("-hasValue")}))}function getStudentCardShowPage(){return`
    ...
    `}function init(){initSidebar()}init();