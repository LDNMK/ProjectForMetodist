CREATE TABLE [dbo].[students_info] (
    [id]                         INT           NOT NULL,
    [birthdate]                  DATE          NOT NULL,
    [birth_place]                NVARCHAR (30) NOT NULL,
    [immenseness]                NVARCHAR (30) NOT NULL,
    [marital_status]             TINYINT       DEFAULT ((1)) NULL,
    [registration]               NVARCHAR (30) NOT NULL,
    [exemption]                  NVARCHAR (30) NOT NULL,
    [expirience_competition]     TINYINT       DEFAULT ((1)) NULL,
    [transfer_from]              NVARCHAR (30) NULL,
    [transfer_direction]         NVARCHAR (30) NULL,
    [competition_conditions]     NVARCHAR (30) NULL,
    [out_of_competition_info]    NVARCHAR (30) NULL,
    [ammends]                    TINYINT       DEFAULT ((1)) NULL,
    [employment_number]          INT           NULL,
    [employment_authority]       NVARCHAR (30) NULL,
    [employment_given_date]      DATE          NULL,
    [registr_or_passport_number] NVARCHAR (50) NULL,
    PRIMARY KEY CLUSTERED ([id] ASC),
    FOREIGN KEY ([ammends]) REFERENCES [dbo].[ammendes] ([id]),
    FOREIGN KEY ([expirience_competition]) REFERENCES [dbo].[expirience_competitiones] ([id]),
    FOREIGN KEY ([id]) REFERENCES [dbo].[students] ([id]),
    FOREIGN KEY ([marital_status]) REFERENCES [dbo].[marital_statuses] ([id])
);

